import numpy as np
import pandas as pd
import em_hmm_directional.vonMisesFisher_distribution as vMF
...
def vonCV_EM(X, K_list, conditions, output_path="./", Ninits=5):
    ...
    for i in range(num_conditions):
        condition = X[i, :]
        df = pd.DataFrame(
            index=K_list, columns=["train", "train_std", "LOO", "LOO_std"]
        )
        for k in K_list:
            test_ll = []
            train_ll = []
            for j in range(num_subjects):
                print("===============================================")
                print(
                    "=======Condition: {}, num_K: {}, subject: {}==============".format(
                        i, k, j
                    )
                )
                print("================================================")
            ...
            df.at[k, "LOO"] = mean(test_ll)
            df.at[k, "train"] = mean(train_ll)
            df.at[k, "LOO_std"] = std(test_ll)
            df.at[k, "train_std"] = std(train_ll)
